...
static volatile bool g_demoShowSquareState = false;

// =====================================
// Pong frame storage (double-buffered)
// =====================================
static XYPoint g_pongFrame[2][PONG_FRAME_MAX_POINTS];
static bool g_pongBlankBefore[2][PONG_FRAME_MAX_POINTS];
...
static bool g_audioBlankBefore[2][AUDIO_FRAME_MAX_POINTS];
...
void drawingBegin() {
  ...
  g_mode = AppMode::ETCH;
  g_demoIndex = 0;
  g_demoShowSquareState = false;

  g_pongFrameCount[0] = 0;
  g_pongFrameCount[1] = 0;
  g_pongActiveBuf = 0;
  g_pongReplayIndex = 0;

  g_audioFrameCount[0] = 0;
  g_audioFrameCount[1] = 0;
  g_audioActiveBuf = 0;
...
void drawingSetPongFrame(const XYPoint* pts, const bool* blankBefore, size_t count) {
  ...
  memcpy(g_pongBlankBefore[writeBuf], blankBefore, count * sizeof(bool));
  g_pongFrameCount[writeBuf] = count;

  // Very short swap lock
  portENTER_CRITICAL(&g_etchMux);
...
// =====================================================
// Audio frame setters (double-buffered)
// =====================================================
...
void drawingSetAudioFrame(const XYPoint* pts, const bool* blankBefore, size_t count) {
  ...
  memcpy(g_audioBlankBefore[writeBuf], blankBefore, count * sizeof(bool));
  ...
  switch (mode) {
    case AppMode::ETCH: {
      ...
      if (count == 0) {
        ...
        if (count > 1 && idx == 0) {
          step.blankBefore = true;
        }

        step.point = g_path[idx];
        g_replayIndex = idx + 1;
      }
      break;
    }
    ...
    case AppMode::SHAPE_DEMO: {
      ...
      if (showSquare != g_demoShowSquareState) {
        g_demoShowSquareState = showSquare;
        idx = 0;
        g_demoIndex = 0;
        step.blankBefore = true;
      }
      ...
      if (DEMO_POINT_COUNT > 1 && idx == 0) {
        step.blankBefore = true;
      }

      step.point = showSquare ? g_demoSquare[idx] : g_demoCircle[idx];
      g_demoIndex = idx + 1;
      break;
    }
    ...
    case AppMode::PONG: {
      ...
      if (count == 0) {
        ...
        step.blankBefore = g_pongBlankBefore[activeBuf][idx];
        step.point = g_pongFrame[activeBuf][idx];
    ...
    case AppMode::USB_STREAM: {
      ...
      if (count == 0) {
        ...
        step.blankBefore = g_audioBlankBefore[activeBuf][idx];
        step.point = g_audioFrame[activeBuf][idx];
...
XYPoint drawingGetNextReplayPoint() {
  return drawingGetNextReplayStep().point;
}